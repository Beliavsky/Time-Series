...
It provides sparse-lag two-regime TAR likelihoods, conjugate Gaussian
...
modal delay selection, and DIC.

Internal or externally supplied threshold variables are supported.

Principal interfaces are available through both `arma_mod` and
`bayesian_time_series_mod`.

## Licensing

The translation is licensed under GPL-2.0-or-later; see `LICENSE-BAYSTAR`.
